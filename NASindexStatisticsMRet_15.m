...
% Leggi i dati dal worksheet "NASindexMRet" mantenendo i nomi originali delle colonne
opts = detectImportOptions(filePath, 'Sheet', 'NASindexMRet');
opts.VariableNamingRule = 'preserve';
data = readtable(filePath, opts);

% Calcola i rendimenti mensili
returns = diff(log(data.('NASDAQ COMPOSITE - TOT RETURN IND')));
...
disp('Statistiche per NASindexMRet:');
disp(statsTable);
...
title('Distribuzione dei Rendimenti e Curva di Gauss per NASindexMRet');
xlabel('Rendimenti');
