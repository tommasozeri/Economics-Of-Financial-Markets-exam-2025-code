...
opts = detectImportOptions('C:\Users\Tomma\MATLAB Drive\EFM EXAM 2025\DBEXAM.xlsx', 'Sheet', 'nasdaq daily');
opts.VariableNamingRule = 'preserve';
nasdaq_data = readtable('C:\Users\Tomma\MATLAB Drive\EFM EXAM 2025\DBEXAM.xlsx', opts);

opts = detectImportOptions('C:\Users\Tomma\MATLAB Drive\EFM EXAM 2025\DBEXAM.xlsx', 'Sheet', 'NASindexD');
opts.VariableNamingRule = 'preserve';
benchmark_data = readtable('C:\Users\Tomma\MATLAB Drive\EFM EXAM 2025\DBEXAM.xlsx', opts);

% Converti le date in formato datetime
nasdaq_data.DATE = datetime(nasdaq_data.DATE, 'InputFormat', 'dd/MM/yyyy');
...
common_dates = intersect(nasdaq_data.DATE, benchmark_data.DATE);

% Filtra i dati per le date comuni
nasdaq_data = nasdaq_data(ismember(nasdaq_data.DATE, common_dates), :);
benchmark_data = benchmark_data(ismember(benchmark_data.DATE, common_dates), :);

% Calcola i rendimenti giornalieri normali
nasdaq_returns = diff(nasdaq_data{:, 2:end}) ./ nasdaq_data{1:end-1, 2:end};
...
stock_names = nasdaq_data.Properties.VariableNames(2:end);
...
for i = 1:num_stocks
    ...
    b = X \ y;
    beta(i) = b(2);
    mean_returns(i) = mean(nasdaq_returns(:, i));
    std_returns(i) = std(nasdaq_returns(:, i));
    ...
    cumulative_returns = cumprod(1 + nasdaq_returns(:, i)) - 1;
...
correlation_matrix = corr(nasdaq_returns, benchmark_returns);
