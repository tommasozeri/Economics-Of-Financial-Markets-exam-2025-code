% Leggi il file Excel
T = readtable('NYSEBLSELECTEDDaily.xlsx', 'VariableNamingRule', 'preserve');

% Definisci i nomi degli asset e del benchmark
assetNames = ["ELI LILLY", "WALMART", "KROGER","CALERES", "FTI CONSULTING", "NEW YORK TIMES 'A'", "MAGNOLIA OIL GAS A", "MURPHY OIL", "BIOHAVEN", "TELEPHONE & DATA SYS."];
...
% Vista 1
P(1, assetNames=="ELI LILLY") = 1; 
q(1) = 0.05;
Omega(1, 1) = 1e-3;

% Vista 2
P(2, assetNames=="WALMART") = 1; 
q(2) = 0.03;
Omega(2, 2) = 1e-3;

% Vista 3
P(3, assetNames=="NEW YORK TIMES 'A'") = 1; 
P(3, assetNames=="FTI CONSULTING") = -1; 
q(3) = 0.05;
Omega(3, 3) = 1e-5;

% Vista 4
P(4, assetNames=="MAGNOLIA OIL GAS A") = 1; 
P(4, assetNames=="MURPHY OIL") = -1; 
q(4) = 0.05;
Omega(4, 4) = 1e-5;
...
betaMV = (assetRetns.Variables * wts) \ benchRetn.Variables;
betaBL = (assetRetns.Variables * wtsBL) \ benchRetn.Variables;
...
% Stampa i pesi dei portafogli
fprintf('\nPesi del Portafoglio Media-Varianza:\n');
for i = 1:length(wts)
    fprintf('%s: %f\n', assetNames(i), wts(i));
end

fprintf('\nPesi del Portafoglio Black-Litterman:\n');
for i = 1:length(wtsBL)
    fprintf('%s: %f\n', assetNames(i), wtsBL(i));
end
