...
% Carica i dati dal file Excel DBEXAM.xlsx, worksheet NYSEselectedM
A = readtable('DBEXAM.xlsx', 'Sheet', 'FUNDSselectedM', 'ReadRowNames', true, 'VariableNamingRule', 'preserve');
tickers = A.Properties.VariableNames;
A = table2array(A);

% Carica i dati di mercato dal file Excel DBEXAM.xlsx, worksheet SP500indexM
MercatoDati = readtable('DBEXAM.xlsx', 'Sheet', 'SP500indexM', 'ReadRowNames', true, 'VariableNamingRule', 'preserve');
...
[x, fval, exitflag, output] = fmincon(@(x) PrimaFunzObbP1EFMM_funds(x), z0, A, B, Aeq, Beq, LB, UB, [], options);
...
annualized_risk = portfolio_risk * sqrt(12);
...
p = Portfolio('AssetList', tickers, 'RiskFreeRate', risk_free_rate); % Tasso risk-free mensile
...
% Grafico della frontiera efficiente (valori mensili)
figure;
...
title('Frontiera Efficiente sul Piano Media-Varianza (Valori Mensili)');
...
% Crea il pie chart
figure;
pie(percent_weights, string(tickers)); % Usa i ticker come etichette
title('Pesi Ottimali del Portafoglio (in percentuale)');
legend(tickers, 'Location', 'bestoutside');