function z = portafoglio2EFM
    ...
    A = readtable('DBEXAM.xlsx', 'Sheet', 'NASselectedD', 'ReadRowNames', true, 'VariableNamingRule', 'preserve');
    tickers = A.Properties.VariableNames;
    A = table2array(A);

    % Carica i dati di mercato dal file Excel DBEXAM.xlsx, worksheet SP500indexD
    MercatoDati = readtable('DBEXAM.xlsx', 'Sheet', 'NASindexD', 'ReadRowNames', true, 'VariableNamingRule', 'preserve');
    ...
    [x, fval, exitflag, output] = fmincon(@(x) secondaFunzObbP2EFMD_7(x), z0, A, B, Aeq, Beq, LB, UB, [], options);
